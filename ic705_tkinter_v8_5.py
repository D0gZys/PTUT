...
# -*- coding: utf-8 -*-
"""
IC-705 Spectrum Display - Version 8.5
=====================================
Version optimisee avec calibration dBm automatique et connexion directe IC-705.
...
from ic705 import IC705Client, IC705Error
...
RADIO_IP_DEFAULT = "192.168.59.1"
RADIO_USERNAME_DEFAULT = "IC-705-7"
RADIO_PASSWORD_DEFAULT = "bouter20xx"
RADIO_NAME_DEFAULT = "IC-705"
RADIO_MAC_DEFAULT = "00:90:C7:13:CA:75"
ADRESSE_RADIO = 0xA4
ADRESSE_PC = 0xE0
...
# Rafraichissement frequence (secondes)
FREQ_POLL_S = 0.5
...
def extraire_spectre(msg):
    ...
    if len(msg) < 10:
        return None
    if msg[0] != CIV_PREAMBLE[0] or msg[1] != CIV_PREAMBLE[1] or msg[-1] != CIV_END[0]:
        return None
    if msg[4] != 0x27:
        return None
    ...
    base = 1 if data[0] == 0x00 else 0
    header_len = base + 15
    if len(data) >= header_len + LARGEUR_SPECTRE:
        raw = data[header_len:header_len + LARGEUR_SPECTRE]
...
def parse_mac_bytes(mac_text):
    cleaned = mac_text.replace(":", "").replace("-", "").replace(" ", "")
    if len(cleaned) != 12:
        raise ValueError("Invalid MAC format")
    try:
        return bytes.fromhex(cleaned)
    except ValueError as exc:
        raise ValueError("Invalid MAC format") from exc
...
class IC705SpectrumV85:
    """Application spectre IC-705 optimisee."""
    
    def __init__(self, root):
        self.root = root
        self.root.title("IC-705 Spectrum v8.5 - dBm calibre")
    ...
    def creer_interface(self):
        ...
        # Titre
        tk.Label(top, text="📡 IC-705 Spectrum v8.5", font=("Helvetica", 16, "bold"),
        ...
        # === Connexion radio ===
        cfg = tk.Frame(self.root, bg='#1a1a2e')
        cfg.pack(fill='x', padx=10, pady=(0, 5))

        tk.Label(cfg, text="Radio IP:", fg='white', bg='#1a1a2e').pack(side='left', padx=(0, 5))
        self.entry_ip = tk.Entry(cfg, width=14)
        self.entry_ip.insert(0, RADIO_IP_DEFAULT)
        self.entry_ip.pack(side='left', padx=(0, 10))

        tk.Label(cfg, text="User:", fg='white', bg='#1a1a2e').pack(side='left', padx=(0, 5))
        self.entry_user = tk.Entry(cfg, width=12)
        self.entry_user.insert(0, RADIO_USERNAME_DEFAULT)
        self.entry_user.pack(side='left', padx=(0, 10))

        tk.Label(cfg, text="Pass:", fg='white', bg='#1a1a2e').pack(side='left', padx=(0, 5))
        self.entry_pass = tk.Entry(cfg, width=12, show="*")
        self.entry_pass.insert(0, RADIO_PASSWORD_DEFAULT)
        self.entry_pass.pack(side='left', padx=(0, 10))

        tk.Label(cfg, text="Name:", fg='white', bg='#1a1a2e').pack(side='left', padx=(0, 5))
        self.entry_radio_name = tk.Entry(cfg, width=12)
        self.entry_radio_name.insert(0, RADIO_NAME_DEFAULT)
        self.entry_radio_name.pack(side='left', padx=(0, 10))

        tk.Label(cfg, text="MAC:", fg='white', bg='#1a1a2e').pack(side='left', padx=(0, 5))
        self.entry_mac = tk.Entry(cfg, width=14)
        self.entry_mac.insert(0, RADIO_MAC_DEFAULT)
        self.entry_mac.pack(side='left', padx=(0, 10))
    ...
    def creer_graphique(self):
        ...
        # Axes fréquence (offset kHz autour de la fréquence centrale)
        demi_span = self.span_khz / 2.0
        self.axe_freq = np.linspace(-demi_span, demi_span, LARGEUR_SPECTRE)
        ...
        self.ax_spec.set_xlabel('Offset (kHz)', color='white')
        self.ax_spec.set_ylabel('Niveau (dBm)', color='white')
        self.ax_spec.set_xlim(-demi_span, demi_span)
        ...
        self.ax_spec.xaxis.set_major_formatter(FuncFormatter(lambda x, p: f'{x:.1f}'))
        
        # Ligne centre
        self.ligne_centre = self.ax_spec.axvline(x=0.0, color='red',
                                                   linestyle='--', alpha=0.7)
    ...
    def connecter(self):
        radio_ip = self.entry_ip.get().strip()
        username = self.entry_user.get().strip()
        password = self.entry_pass.get()
        radio_name = self.entry_radio_name.get().strip() or RADIO_NAME_DEFAULT
        mac_text = self.entry_mac.get().strip()

        if not radio_ip:
            messagebox.showerror("Erreur", "IP radio invalide")
            return
        if not username or not password:
            messagebox.showerror("Erreur", "Identifiants manquants")
            return

        radio_mac = None
        if mac_text:
            try:
                radio_mac = parse_mac_bytes(mac_text)
            except ValueError:
                messagebox.showerror("Erreur", "MAC invalide (ex: 001122AABBCC)")
                return

        try:
            self.client = IC705Client(
                radio_ip=radio_ip,
                username=username,
                password=password,
                radio_name=radio_name,
                radio_mac=radio_mac,
                civ_to=ADRESSE_RADIO,
                civ_from=ADRESSE_PC,
            )
            self.client.connect()
            self.client.enable_scope_output(True)

            freq_hz = self.client.get_frequency(timeout_s=1.0)
            if freq_hz:
                self.freq_centrale = freq_hz / 1_000_000
                self.maj_axe_freq()
        ...
        except (IC705Error, Exception) as e:
    ...
    def deconnecter(self):
        ...
        if self.client:
            try:
                self.client.enable_scope_output(False)
            except Exception:
                pass
            try:
                self.client.close()
    ...
    def rafraichir_freq(self):
        """Demande la frequence actuelle via CI-V."""
        if not self.client:
            return
        try:
            freq_hz = self.client.get_frequency(timeout_s=0.2)
        except Exception:
            return
        if freq_hz:
            self.nouvelle_freq = freq_hz / 1_000_000

    def boucle_reception(self):
        """Thread de réception des données."""
        compteur_freq = 0
        last_freq_poll = time.time()
        
        while self.actif and self.connecte and self.client:
            try:
                payload = self.client.get_spectrum_frame(timeout_s=0.2)
            except Exception:
                break

            if time.time() - last_freq_poll >= FREQ_POLL_S:
                self.rafraichir_freq()
                last_freq_poll = time.time()
            ...
            compteur_freq = 0
            amplitudes = extraire_spectre(payload)
            if amplitudes is None or len(amplitudes) <= 10:
                continue
    ...
    def maj_axe_freq(self):
        ...
        self.ligne_centre.set_xdata([0.0, 0.0])
        self.ax_spec.set_xlim(-demi_span, demi_span)
        self.ax_spec.set_title(f'Spectre - {self.freq_centrale:.3f} MHz', color='white')
        
        self.img_wf.set_extent([-demi_span, demi_span, PROFONDEUR_WATERFALL, 0])
        self.ax_wf.set_xlim(-demi_span, demi_span)
    ...
    def ecrire_csv(self, spectre, timestamp, freq, ref_level):
        ...
        try:
            ligne = [timestamp, f"{freq:.6f}", f"{self.span_khz:.3f}", ref_level]
