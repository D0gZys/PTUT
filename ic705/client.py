...
class IC705Client:
    ...
    def connect(self, timeout_s: float = 20.0) -> None:
        ...
        if self.civ_remote_port == 0:
            raise IC705Error("CIV port is 0 (streams not granted).")
        self._open_civ()
    ...
    def send_civ_frame(self, civ_payload: bytes) -> None:
        if not self.civ_stream:
            raise IC705Error("CIV stream not ready.")
    ...
    def get_frequency(self, timeout_s: float = 1.0) -> Optional[int]:
        ...
        while time.time() < deadline:
            ...
            if len(data) != 5:
                continue
            return bcd_lsb_bytes_to_int(data)
        return None
    ...
    def get_spectrum_frame(self, timeout_s: float = 0.2) -> Optional[bytes]:
        deadline = time.time() + timeout_s
        while time.time() < deadline:
            payload = self.recv_civ(timeout_s=deadline - time.time())
            if not payload:
    ...
    def set_scope_on(self, enable: bool = True) -> None:
        self.send_civ_cmd(0x27, bytes([0x10, 0x01 if enable else 0x00]))

    def set_scope_data_output(self, enable: bool = True) -> None:
        self.send_civ_cmd(0x27, bytes([0x11, 0x01 if enable else 0x00]))

    def set_scope_hold(self, hold: bool = False) -> None:
        self.send_civ_cmd(0x27, bytes([0x17, 0x01 if hold else 0x00]))

    def enable_scope_output(self, enable: bool = True) -> None:
        self.set_scope_on(enable)
        self.set_scope_data_output(enable)
        if enable:
            self.set_scope_hold(False)
    ...
    def _control_handshake(self, timeout_s: float) -> None:
        if not self.ctrl_stream:
            raise IC705Error("Control stream not open.")
        t0 = time.time()
        next_ayt = 0.0
        next_idle = 0.0
        ...
        while time.time() - t0 < timeout_s:
            ...
            if len(r) == CONTROL_SIZE:
                ...
                if error == 0xFEFFFFFF:
                    raise IC705Error("Invalid Username/Password (0xFEFFFFFF)")
                ...
                if error == 0xFFFFFFFF:
                    raise IC705Error("Connection failed (status error=0xFFFFFFFF).")
                if error == 0x00000000 and disc == 0x01:
                    raise IC705Error("Radio reports disconnected (disc=0x01).")
                self.civ_remote_port = civ_port
                return

        raise IC705Error("Timeout during control handshake.")

    def _civ_handshake(self, timeout_s: float) -> None:
        if not self.civ_stream:
            raise IC705Error("CIV stream not open.")
        t0 = time.time()
        next_ayt = 0.0
    ...
    def _keepalive_loop(self) -> None:
        next_idle = time.time() + IDLE_PERIOD
        next_token = time.time() + TOKEN_RENEWAL
        ...
        while not self._keepalive_stop.is_set():
            ...
            if now >= next_token:
                try:
                    ...
                    pass
                next_token = now + TOKEN_RENEWAL

            try:
                data, _ = self.ctrl_stream.sock.recvfrom(4096)
            except socket.timeout:
                continue
            except OSError:
                break
            ...
            if len(data) == PING_SIZE:
                self._handle_ping(self.ctrl_stream, data, CONTROL_PORT)
                continue
