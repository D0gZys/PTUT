...
class IC705AppV7:
    ...
    def toggle_enregistrement(self):
        """Demarre ou arrete l'enregistrement CSV."""
    ...
    def _push_trigger_prebuffer(self, spectre, timestamp, freq_mhz, span_khz):
        """Stocke une ligne dans le buffer avant trigger."""
        if TRIGGER_PRE_LINES <= 0:
            return
        self.trigger_pre_buffer.append((timestamp, freq_mhz, span_khz, spectre.copy()))

    def _flush_trigger_prebuffer(self):
        """Ecrit les lignes tampon avant trigger."""
        if not self.writer_csv:
            return
        for ts, freq, span, spectre in self.trigger_pre_buffer:
            self.ecrire_ligne_csv(spectre, timestamp=ts, freq_mhz=freq, span_khz=span)
        self.trigger_pre_buffer.clear()
            ...
            if max_signal >= seuil:
                if not self.au_dessus_seuil:
                    ...
                    if not self.writer_csv:
                        ...
                        self._flush_trigger_prebuffer()
                # Si on repasse le seuil pendant le post-trigger, on garde le meme CSV.
                self.post_trigger_restantes = TRIGGER_POST_LINES

                if self.writer_csv:
                    self.ecrire_ligne_csv(spectre, timestamp=timestamp, freq_mhz=freq_mhz, span_khz=span_khz)
            else:
    ...
    def ecrire_ligne_csv(self, spectre, timestamp=None, freq_mhz=None, span_khz=None):
        ...
        try:
            ...
            freq = self.freq_centrale if freq_mhz is None else float(freq_mhz)
            span = SPAN_KHZ if span_khz is None else int(span_khz)
            ligne = [ts, f"{freq:.6f}", span]
